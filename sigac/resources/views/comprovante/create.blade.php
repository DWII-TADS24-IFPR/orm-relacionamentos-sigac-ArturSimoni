        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('coordenador.comprovantes.store') }}" method="POST" class="space-y-4">
                        @csrf

                        <div>
                            <label for="horas" class="block text-sm font-medium">Horas</label>
                            <input type="number" name="horas" id="horas" value="{{ old('horas') }}" class="mt-1 block w-full rounded border-gray-300 dark:bg-gray-700 dark:text-gray-100" required>
                            @error('horas')
                        ...
                        <div>
                            ...
                            <input type="text" name="atividade" id="atividade" value="{{ old('atividade') }}" class="mt-1 block w-full rounded border-gray-300 dark:bg-gray-700 dark:text-gray-100" required>
                            @error('atividade')
                                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                            @enderror
                        </div>
